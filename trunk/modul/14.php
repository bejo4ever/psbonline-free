...
if(isset($_POST['submit'])){
	...
	$result = mysql_query("SELECT * FROM pendaftar where (nopendaftaran like '%$query_cari%' or nama like '%$query_cari%') and dipakai='0'");
?>

<h2>Validasi Pendaftaran Siswa Baru</h2>
<p><em>Silakan klik link 'validasi' pada samping kanan nama Calon Siswa yang memvalidasi pendaftarannya.</em></p>
...
$no = 0; $no = $no+1;
	while($row = mysql_fetch_array($result))
	  ...
	  <?php
		...
		echo "<td>";?>
		<a href="?p=15&i=<?php echo $row['id'];?>">Validasi</a>
		<?php echo "</td>";
		echo '</tr>';
		$no++;
	  }
