...
<?php
session_start();
echo "<h2>Pembatalan Pendaftaran</h2>";
if(isset($_GET['p']) && isset($_GET['i']) && isset($_SESSION['session_login'])){
	$id_calon = filterinput($_GET['i']);
	$query = mysql_query("UPDATE pendaftar SET dipakai = '0' WHERE id='$id_calon' limit 1");
	if($query){
		echo "<p><strong>Sukses</strong>, membatalkan pendaftaran peserta dengan ID: $id_calon</p>";
	}else{
		echo "<p><strong>Gagal</strong>, membatalkan pendaftaran peserta dengan ID: $id_calon. Silakan ulangi lagi.</p>";
		}
